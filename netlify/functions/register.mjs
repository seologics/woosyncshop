...
export default async (req) => {
  ...
  try {
    ...
    if (user?.id && metadata) {
      const isPaidPlan = metadata.plan && metadata.plan !== 'free_forever';
      await supabase.from('user_profiles').upsert({
        ...
        // Paid plans start as pending_payment until Mollie webhook confirms
        plan: isPaidPlan ? 'pending_payment' : (metadata.plan || 'pending_payment'),
        chosen_plan: isPaidPlan ? metadata.plan : null,
...
<body style="margin:0;padding:0;background:#0f1117;font-family:'DM Sans',Arial,sans-serif;color:#e2e8f0">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f1117;padding:40px 0">
    ...
    // ── Payment reminder email for paid plans (account created, payment pending) ──
    if (metadata?.plan && metadata.plan !== 'free_forever') {
      try {
        const firstName = (metadata.full_name || email).split(' ')[0]
        const planNames = { starter: 'Starter', growth: 'Growth', pro: 'Pro' }
        const planName = planNames[metadata.plan] || 'Growth'
        const billingLabel = metadata.billing_period === 'annual' ? 'jaarabonnement' : 'maandabonnement'
        ...
        const priceKey = metadata.billing_period === 'annual' ? 'annual_mo' : 'monthly'
        const price = planPrices[metadata.plan]?.[priceKey] || '11.99'
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#1a1d2e;border-radius:12px;overflow:hidden;border:1px solid #2d3056">
        ...
        <tr><td style="padding:36px 40px">
          ...
          <p style="margin:0 0 20px;font-size:14px;line-height:1.7;color:#94a3b8">
            Je hebt je geregistreerd voor <strong style="color:#c4b5fd">WooSyncShop ${planName}</strong> (${billingLabel} · €${price}/maand).
            Er is alleen nog één stap over: je betaling afronden.
          </p>
          ...
          <p style="margin:0 0 16px;font-size:13px;line-height:1.7;color:#94a3b8">
            Log in op je account om de betaling af te ronden. Je kunt ook van pakket wisselen als je dat wilt.
          </p>
        ...
        await supabase.from('system_logs').insert({
          level: 'info', function_name: 'register',
          message: `Payment reminder email sent to ${email} (plan: ${metadata.plan})`,
          metadata: { user_id: user?.id, plan: metadata.plan },
        })
      } catch (mailErr) {
        await supabase.from('system_logs').insert({
          level: 'warn', function_name: 'register',
          message: `Payment reminder email failed for ${email}: ${mailErr.message}`,
          metadata: { user_id: user?.id },
        })
      }
    }
